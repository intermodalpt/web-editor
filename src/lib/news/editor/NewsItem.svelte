<script lang="ts">
	...
	import { getExternalNewsItem, createNewsItem } from '$lib/api';
	...
	import ExternalSourceRow from '../row/ExternalSource.svelte';
	import {
		defaultContentBlock,
		nonBlankString,
		externalContentToContentBlocks
	...
	const original = item;
	const canEdit =
		(original && $permissions?.news?.modify) || (!original && $permissions?.news?.modify);
	...
	let selectedRegions = original
		? original.region_ids.map((id) => {
				return { value: id };
			})
		: [];
	let selectedOperators = original
		? original.operator_ids.map((id) => {
				return { value: id };
			})
		: [];

	let title: string = original ? original.title : '';
	let summary: string | null = original ? original.summary : '';
	let is_visible = original ? original.is_visible : true;
	let author_override: string | null = original ? original.author_override : null;
	let pubDatetime = original ? stripTimezone(original.publish_datetime) : null;
	let editDatetime = original ? stripTimezone(original.edit_datetime) : null;
	let content = original ? original.content : [];
	let externalIds = original ? original.external_rels.map((rel) => rel.id) : [];
	let images = {};
...
<h2 class="flex gap-2 items-center">
	<div class="form-control grow">
		<label class="input-group">
			<span class="label-text text-slate-500">Titulo</span>
			<input
				type="text"
				bind:value={title}
...
<textarea
	...
	disabled={!canEdit}
></textarea>
<span class="label-text text-slate-500">Miniatura</span>
<div class="flex gap-2 flex-wrap ml-2">
	...
	{#each Object.values(images ?? {}) as img}
		<button class="relative" on:click={() => (thumbId = img.id)}>
			<img
				src={img.url}
				alt={img.transcript}
...
<div class="flex py-2 gap-4 flex-wrap">
	<div class="form-control">
		<label class="input-group">
			...
			<input type="datetime-local" bind:value={pubDatetime} class="input input-bordered input-sm" />
		</label>
	</div>
	<div class="form-control">
		<label class="input-group">
...
<div class="flex py-2 gap-4 flex-wrap items-center">
	<div class="form-control">
		<label class="input-group">
